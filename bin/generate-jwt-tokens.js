...
function main() {
  ...
  // Use JWT secret from environment or command line argument
  const jwtSecret = process.env.JWT_SECRET || process.argv[2];
  
  if (!jwtSecret) {
    console.error('❌ JWT_SECRET not found. Please set JWT_SECRET environment variable or provide as command line argument.');
    process.exit(1);
  }
  ...
  console.log('Using JWT secret from:', process.env.JWT_SECRET ? '.env file' : 'command line');
